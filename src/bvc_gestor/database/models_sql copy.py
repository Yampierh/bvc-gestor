...
class CuentaBancariaDB(Base, AuditMixin):
    ...
    numero_cuenta: Mapped[str] = mapped_column(String(20), nullable=False) # Número de cuenta bancaria
...
class CuentaBursatilDB(Base, AuditMixin):
    ...
    cuenta: Mapped[str] = mapped_column(String(50), nullable=False) # Numero de Cuenta
...
class TituloDB(Base, AuditMixin):
    ...
    nombre: Mapped[str] = mapped_column(String(150), nullable=False) # Nombre completo del titulo
    ticker: Mapped[str] = mapped_column(String(10), unique=True, nullable=False) # Símbolo en la bolsa
    sector: Mapped[str] = mapped_column(String(100), nullable=True) # Sector económico
    mercado: Mapped[str] = mapped_column(String(10), default='Acciones')  # 'Acciones', 'Bonos', 'ETF'
    tipo: Mapped[str] = mapped_column(String(15), default='Renta Variable') # 'Renta Variable', 'Renta Fija'
    precio_actual: Mapped[Optional[Decimal]] = mapped_column(DECIMAL(20, 8), nullable=True) # Precio del titulo
    fecha_actualizacion_precio: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    estado_mercado: Mapped[bool] = mapped_column(Boolean, default=True)  # titulo/Suspendido en BVC

    # Relaciones
    ordenes = relationship("OrdenDB", back_populates="titulo")
    portafolio_items = relationship("PortafolioItemDB", back_populates="titulo")
    precios_actuales = relationship("PrecioActualDB", back_populates="titulo")
    precios_historicos = relationship("PrecioHistoricoDB", back_populates="titulo")
...
class OrdenDB(Base, AuditMixin):
    ...
    titulo_id: Mapped[str] = mapped_column(ForeignKey("titulos.id"), nullable=False)
...
class PortafolioItemDB(Base, AuditMixin):
    """Resumen consolidado de titulo por cuenta"""
    __tablename__ = "portafolio_items"
    
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    cuenta_id: Mapped[int] = mapped_column(ForeignKey("cuentas_bursatiles.id"), nullable=False)
    titulo_id: Mapped[str] = mapped_column(ForeignKey("titulos.ticker"), nullable=False)
...
class PrecioActualDB(Base, AuditMixin):
    ...
    # Relaciones
    titulo = relationship("TituloDB", back_populates="precios_actuales")
    
    __table_args__ = (
        Index('idx_precio_actual_titulo', 'titulo_id'),
...
class PrecioHistoricoDB(Base):
    ...
    # Relaciones
    titulo = relationship("TituloDB", back_populates="precios_historicos")
    
    __table_args__ = (
        Index('idx_precio_historico_titulo_fecha', 'titulo_id', 'fecha'),
        UniqueConstraint('titulo_id', 'fecha', name='uq_titulo_fecha'),
...
class ConfiguracionComisionesDB(Base, AuditMixin):
    ...
    titulo: Mapped[bool] = mapped_column(Boolean, default=True)
